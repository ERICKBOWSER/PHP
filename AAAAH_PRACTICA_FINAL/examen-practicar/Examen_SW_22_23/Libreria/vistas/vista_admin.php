...
if(isset($_POST["btnAgregar"])){
    ...
    if(!$error_referencia){
        ...
        if(!$obj){
            session_destroy();
            die(errorPage("Examen practicar", "<h1>Libreria</h1><p>Error consumiendo el servicio: " . $url . "</p>"));
        }
        if(isset($obj->error)){
            session_destroy();
            die(errorPage("Examen practicar", "<h1><Libreria</h1><p>" . $obj->error . "</p>"));
        }
    ...
    $array_nombre = explode(".", $_FILES["portada"]["name"]); // DIVIDE UN STRING Y DEVUELVE UN ARRAY
    // getimagesize determina el tamaño de un fichero de imagen
    ...
    // SI NO HAY ERRORES EN EL FORMULARIO Y QUEREMOS AÑADIR UN NUEVO LIBRO
    if(!$error_form){
        ...
        if(!$obj){
            session_destroy();
            die(errorPage("Examen practicar", "<h1>Librería</h1><p>Error consumiendo el servicio; " . $url . "</p>"));
        }

        if(isset($obj->error)){
            session_destroy();
            die("Examen practicar", "<h1>Librería</h1><p>" . $obj->error . "</p>");
        }
        ...
        if($_FILES["portada"]["name"] !=""){
            $ext = end($array_nombre); // end() devuelve el último valor de un array
            $nombre_nuevo = "img" . $_POST["referencia"] . "." . $ext;
            // move_uploaded_file() para movere ficheros
            ...
            if($var){
                ...
                if(isset($obj->error)){
                    unlink("../img/" . $nombre_nuevo); // unlink BORRA UN FICHERO
                    session_unset();
                    die(errorPage("Examen practicar", "<h1>Librería</h1><p>" . $obj->error . "</p>"));
                }
                if(isset($obj->no_auth)){
                    unlink("../img/" . $nombre_nuevo);
                    session_unset();
        ...
        header("Location:gest_libros.php"); // UNA VEZ AÑADIDO EL LIBRO SALIMOS
        exit;
    }
}

if(isset($_POST["btnBorrar"])){
    $_SESSION["accion"] = "El libro con referencia " . $_POST["btnBorrar"] . " se ha borrado correctamente";
    ...
    foreach($obj->libros as $tupla){
        ...
        echo "<td>
            <form action='gest_libros.php' method='post'>
                <button class='enlace' name='btnBorrar' value='" . $tupla->referencia . "'>Borrar</button> -
                <button class='enlace' name='btnEditar' value='" . $tupla->referencia . "'>Editar</button>
            </form></td>";
    }
    echo "</table>";
    ?>
