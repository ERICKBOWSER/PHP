...
// SI NO RECUERDO MAL SI SE NOS OLVIDA COLOCAR EL ; AL FINAL DABA FALLO

// OBTIENE LOS PARAMETROS DE LOS DATOS QUE SE ENVIAN POR LA URL
...
$app->get('/notasAlumno/{cod_usu}', function($request){
    ...
    if(isset($_SESSION["user"])){
        ...
        session_destroy();
        $respuesta["no_auth"]="No tienes permisos para usar este servicio desde /notasAlumno";
        echo json_encode($respuesta);
    }
});
...
$app->get("/alumnos", function($request){
    ...
    if(isset($_SESSION["user"]) && $_SESSION["type"]=="tutor"){
        ...
        session_destroy();
        $respuesta["no_auth"]="No tienes permisos para usar este servicio desde /alumnos";
        echo json_encode($respuesta);
    }
});
...
$app->delete("/quitarNota/{cod_usu}", function($request){
    ...
    session_start();
    if(isset($_SESSION["user"]) && $_SESSION["type"] == "tutor"){
        echo json_encode(quitar_nota_alumno_asig($request->getAttribute("cod_usu"), $request->getParam("cod_asig"))); // POR QUE SE DIFERENCIAN?!
    }else{
        session_destroy();
        $respuesta["no_auth"]="No tienes permisos para usar este servicio /quitarNota";
        echo json_encode($respuesta);
    }
});
...
$app->post("/ponerNota/{cod_usu}", function($request){
    ...
    session_start();
    if(isset($_SESSION["user"]) && $_SESSION["type"]=="tutor"){
        echo json_encode(poner_nota_asig($request->getAttribute("cod_usu"), $request->getParam("cod_asig"))); // getAttribute para el {cod_algo} que se pase por la url, el resto son los datos pasador por el form
    }else{
        session_destroy();
        $respuesta["no_auth"]="No tienes permisos para usar este servicio /ponerNota";
        echo json_encode($respuesta);
    }
});
