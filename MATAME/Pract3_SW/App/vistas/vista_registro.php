...
if(isset($_POST["btnBorrar"])){
    unset($_POST); // DESTRUYE LA VARIABLE
}
...
if(isset($_POST["btnEnviar"])){
    ...
    if(!$error_usuario){
        ...
        if(!$json){
            session_destroy();
            die(error_page("Práctica 3", "<h1>Práctica 3</h1><p>Sin respuesta oportuna de la API desde btnEnviar</p>"));
        }

        if(isset($json["error_bd"])){
            session_destroy();
            consumir_servicios_REST(DIR_SERV."/salir","POST",$datos_env);
            die(error_page("Practica 3", "<h1>Práctica 3</h1><p>".$json["error_bd"]."</p>"));
        }
    ...
    if(!$error_dni){
        ...
        if(!$json){
            session_destroy();
            die(error_page("Practica 3", "<h1>Practica 3</h1><p>Sin respuesta oportuna de la API.</p>"));
        }
    ...
    if(!$error_form){
        ...
        if(!$arrRespuesta){
            session_destroy();
            die(error_page("Practica 3", "<h1>Practica 3</h1><p>El servicio recaptcha no esta disponible.</p>"));
        }
        ...
        if(!$error_captcha){
            ...
            $datos_env_insert["dnid¡"]=strtoupper($_POST["dni"]);
            ...
            if(!$json){
                session_destroy();
                consumir_servicios_REST(DIR_SERV."/salir", "POST", $datos_env);
                die(error_page("Practica 3", "<h1>Practica 3</h1><p>Sin respuesta oportuna de la API desde VISTA_REGISTRO-btnEnviar</p>"));
            }
            ...
            if($_FILES["foto"]["name"]!=""){
                ...
                $ext=".".end($array_ext);
                $nombre_nuevo="img_".$ultm_id.$ext; // NOMBRE NUEVO DE LA IMAGEN
            ...
            // DOS OPCIONES: SIN LOGUEARSE O LOGUEANDO 
            ...
            if(!$json){
                session_destroy();
                die(error_page("Practica 3", "<h1>Práctica 3</h1><p>Sin respuesta oportuna de la API</p>"));
            }
...
<body>
    ...
    <form id="form" action="index.php" method="post" enctype="multipart/form-data">
        ...
        <p>
            ...
            <input type="text" id="dni" name="dni" value="<?php if(isset($_POST["dni"])) echo $_POST["dni"];?>" placeholder="DNI: 12345678Z">
            <?php
        ...
        <p>
            <label>Sexo: </label><br>
            <input type="radio" id="nombre" name="sexo" value="hombre" <?php if(!isset($_POST["sexo"]) || (isset($_POST["sexo"])  && $_POST["sexo"]=="hombre")) echo "checked";?>>
